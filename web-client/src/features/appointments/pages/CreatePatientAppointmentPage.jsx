...
import { CalendarPlus, Clock3, Loader2, Stethoscope, UserRound } from 'lucide-react'
...
export default function CreatePatientAppointmentPage() {
  ...
  const patientAppointmentsParams = useMemo(() => ({ patientId: userId }), [userId])
  ...
  const loadingAppointments = appointmentsStatus === 'idle' || appointmentsStatus === 'loading'
  ...
  const filteredDoctors = useMemo(() => {
    ...
    return normalizedDoctors.filter((doctor) => String(doctor?.specialty || '').toLowerCase() === String(form.specialty).toLowerCase())
  }, [normalizedDoctors, form.specialty])

  const selectedDoctor = useMemo(() => {
    if (!form.doctorId) return null
    return normalizedDoctors.find((doctor) => doctor.appointmentDoctorId === form.doctorId) || null
  ...
  const handleDoctorChange = (nextDoctorId) => {
    const doctor = normalizedDoctors.find((entry) => entry.appointmentDoctorId === nextDoctorId)
    setForm((prev) => ({
      ...prev,
  ...
  const handleSubmit = async (event) => {
    ...
    if (!patientName) {
      setError('Please complete your patient profile name before creating an appointment.')
      return
    }
    if (!form.doctorId) {
    ...
    const doctor = normalizedDoctors.find((entry) => entry.appointmentDoctorId === form.doctorId)
    if (!doctor) {
    ...
    const payload = {
      ...
      doctorSpecialty: String(doctor?.specialty || form.specialty || 'General').trim(),
      patientName,
    ...
    try {
      ...
      setSuccess(`Appointment created successfully${response?.id ? ` (ID: ${response.id})` : ''}.`)
      setForm((prev) => ({ ...prev, scheduledAt: '', reason: '' }))
      await dispatch(fetchAppointments({ params: { patientId: userId }, force: true }))
    } catch (e) {
      setError(formatApiError(e))
    }
  }
    ...
    <div className="space-y-6 w-full">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6 items-start">
        <div className="rounded-xl border p-5" style={{ backgroundColor: 'hsl(var(--card))', borderColor: 'hsl(var(--border))' }}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <select
                ...
                style={{ borderColor: 'hsl(var(--border))', backgroundColor: 'hsl(var(--input))', color: 'hsl(var(--foreground))' }}
            ...
            <div>
              ...
              >
                ...
                {filteredDoctors.map((doctor) => (
                  <option key={doctor.appointmentDoctorId} value={doctor.appointmentDoctorId}>
                    {doctor.fullName || doctor.email || doctor.appointmentDoctorId}
          ...
          {selectedDoctor ? (
            <div className="rounded-lg border px-3 py-2 text-sm" style={{ borderColor: 'hsl(var(--border))', backgroundColor: 'hsl(var(--secondary))', color: 'hsl(var(--foreground))' }}>
              Booking with <span className="font-medium">{selectedDoctor.fullName || selectedDoctor.email || selectedDoctor.appointmentDoctorId}</span>
          ...
          {error ? (
            <div className="rounded-lg border px-3 py-2 text-sm" style={{ borderColor: 'hsl(var(--destructive))', color: 'hsl(var(--destructive))' }}>
              {error}
            </div>
          ) : null}

          {success ? (
            <div className="rounded-lg border px-3 py-2 text-sm" style={{ borderColor: 'hsl(var(--primary))', color: 'hsl(var(--primary))' }}>
              {success} <Link className="underline" to="/patient/telemedicine">View appointments</Link>
            </div>
          ) : null}
          ...
          <button
            ...
            disabled={loadingOptions || submitting}
          >
            {submitting ? <Loader2 size={16} className="animate-spin" /> : <CalendarPlus size={16} />}
        ...
        <section className="rounded-xl border p-5 space-y-4 xl:sticky xl:top-4" style={{ backgroundColor: 'hsl(var(--card))', borderColor: 'hsl(var(--border))' }}>
          ...
          {loadingAppointments ? (
            <div className="space-y-2">
              <div className="h-20 rounded-lg animate-pulse" style={{ backgroundColor: 'hsl(var(--secondary))' }} />
              <div className="h-20 rounded-lg animate-pulse" style={{ backgroundColor: 'hsl(var(--secondary))' }} />
              <div className="h-20 rounded-lg animate-pulse" style={{ backgroundColor: 'hsl(var(--secondary))' }} />
            </div>
          ) : placedAppointments.length === 0 ? (
            <div className="rounded-xl border p-4 text-sm" style={{ borderColor: 'hsl(var(--border))', backgroundColor: 'hsl(var(--secondary))', color: 'hsl(var(--muted-foreground))' }}>
              No appointments placed yet. Create your first appointment from the form on the left.
            </div>
          ) : (
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      ...
                      <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-x-3 gap-y-2">
                        <div className="inline-flex items-center gap-1.5 text-xs" style={{ color: 'hsl(var(--muted-foreground))' }}>
                          <UserRound size={13} />
                          Doctor: {appointment.doctorName || appointment.doctorId || 'Unknown doctor'}
                        </div>
                        <div className="inline-flex items-center gap-1.5 text-xs" style={{ color: 'hsl(var(--muted-foreground))' }}>
                          <Stethoscope size={13} />
                        ...
                        <div className="inline-flex items-center gap-1.5 text-xs" style={{ color: 'hsl(var(--muted-foreground))' }}>
                          <Clock3 size={13} />
                          Scheduled: {formatAppointmentDate(appointment.scheduledAt)}
                        </div>
                        <div className="inline-flex items-center gap-1.5 text-xs" style={{ color: 'hsl(var(--muted-foreground))' }}>
                          <CalendarPlus size={13} />
                          Created: {formatAppointmentDate(appointment.createdAt)}
                        </div>
                      </div>
                    ...
                    <span className="inline-flex items-center rounded-full px-2.5 py-1 text-xs font-semibold" style={appointmentStatusStyles(appointment.status).style}>
