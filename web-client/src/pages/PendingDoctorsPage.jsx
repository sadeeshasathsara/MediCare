...
export default function PendingDoctorsPage() {
    ...
    const loadPending = useCallback(async () => {
        ...
        try {
            ...
            if (!err?.response) {
                ...
                setError(`Failed to load pending doctors${status ? ` (HTTP ${status})` : ''}.`)
            }
        } finally {
            setLoading(false)
        }
    ...
    const verifyDoctor = async (doctorUserId, decision) => {
        ...
        try {
            ...
            setSuccess(decision === 'APPROVE' ? 'Doctor approved.' : 'Doctor rejected.')
            await loadPending()
        } catch (err) {
            const status = err?.response?.status
            const data = err?.response?.data
                                    ...
                                    <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                                        <div>
                                            ...
                                            <div className="text-xs mt-2" style={{ color: 'hsl(var(--muted-foreground))' }}>
                                                License: {profile.licenseNumber || '—'} · Specialty: {profile.specialty || '—'} · Phone:{' '}
                                                {profile.phone || '—'}
                                            </div>
                                        </div>
                                        ...
                                        <div className="flex items-center gap-2">
                                            ...
                                            <button
                                                ...
                                                disabled={busy}
                                                className="px-3 py-2 rounded-xl text-sm font-medium text-white transition-all cursor-pointer"
                                                style={{ backgroundColor: 'hsl(var(--destructive))', opacity: busy ? 0.7 : 1 }}
